...
from src.run import _configure_deterministic_runtime
...
class ConfigOutputTests(unittest.TestCase):
    ...
    def test_cuda_runtime_unhides_empty_container_selector(self) -> None:
        cuda_config = load_config(ROOT / "configs/articraft_microwave_franka.json")
        cpu_config = load_config(ROOT / "configs/microwave_franka.json")
        old = os.environ.get("CUDA_VISIBLE_DEVICES")
        was_present = "CUDA_VISIBLE_DEVICES" in os.environ
        try:
            os.environ["CUDA_VISIBLE_DEVICES"] = ""
            _configure_deterministic_runtime(cuda_config)
            self.assertNotIn("CUDA_VISIBLE_DEVICES", os.environ)
            _configure_deterministic_runtime(cpu_config)
    ...
    def test_bad_quaternion_and_invalid_runtime_modes_are_rejected(self) -> None:
        invalid = copy.deepcopy(self.data)
        ...
        cuda = copy.deepcopy(self.data)
        cuda["runtime"]["device"] = "cuda:0"
        validate_config(cuda)

        invalid = copy.deepcopy(self.data)
        invalid["runtime"]["device"] = "metal"
        with self.assertRaises(PipelineError):
            validate_config(invalid)
