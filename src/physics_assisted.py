...
The robot is a prescribed kinematic body driver, not a torque-controlled
physics robot.  Every robot link pose is generated by the project's name-based
URDF forward kinematics and scattered only to the corresponding Newton
``body_q``/``body_qd`` indices.  Robot inverse mass/inertia are zero and its
body flags are kinematic.  The microwave bodies remain dynamic and are never
written by the driver; in particular no runtime path writes the door's
coordinate, velocity, target, or generalized force.
...
if wp is not None:
    ...
    @wp.kernel
    def _scatter_indexed_body_state(
        indices: wp.array(dtype=wp.int32),
        poses: wp.array(dtype=wp.transform),
        velocities: wp.array(dtype=wp.spatial_vector),
        destination_poses: wp.array(dtype=wp.transform),
        destination_velocities: wp.array(dtype=wp.spatial_vector),
    ):
        controlled_body = wp.tid()
        body_index = indices[controlled_body]
        destination_poses[body_index] = poses[controlled_body]
        destination_velocities[body_index] = velocities[controlled_body]

else:
    _scatter_indexed_body_state = None


_SUPPORTED_NEWTON_VERSION_PREFIX = "1.3."
_GRASP_JOINT_LABEL = "agentpre_fixed_grasp"
_GRASP_ACTIVATION_POSITION_GATE_M = 0.015
_GRASP_ACTIVATION_ORIENTATION_GATE_DEG = 7.5
...
class FixedGraspActivationGate:
    ...
    def passed(self) -> bool:
        return (
            self.position_error_m <= _GRASP_ACTIVATION_POSITION_GATE_M
            and self.orientation_error_deg
            <= _GRASP_ACTIVATION_ORIENTATION_GATE_DEG
        )
    ...
    def audit_metadata(self) -> dict[str, Any]:
        return {
            ...
            "grasp_activation_gate_position_limit_m": _GRASP_ACTIVATION_POSITION_GATE_M,
            "grasp_activation_gate_orientation_limit_deg": _GRASP_ACTIVATION_ORIENTATION_GATE_DEG,
...
class PhysicsParameters:
    """All model, kinematic driver, collision, and stepping inputs from config."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.robot_control_backend.lower() != "kinematic_body_driver":
            raise ValueError(
                "physics-assisted backend requires robot_control_backend='kinematic_body_driver'"
            )
        if self.collision_broad_phase not in {"sap", "nxn", "explicit"}:
            raise ValueError("unsupported Newton collision broad phase")
    ...
    def from_project_config(cls, config: Any) -> "PhysicsParameters":
        ...
        return cls(
            ...
            robot_control_backend=str(
                config.get("simulation.robot_control.backend")
            ),
            collision_enabled=config.get("collision.enabled"),
            collision_broad_phase=str(config.get("collision.broad_phase")),
...
class PhysicsRollout:
    ...
    robot_driver_arm_joint_q: np.ndarray
    ...
    def as_arrays(self) -> dict[str, np.ndarray]:
        return {
            ...
            "robot_driver_arm_joint_q": self.robot_driver_arm_joint_q,
...
class _PhysicsRuntime:
    ...
    robot_body_indices_ordered: tuple[int, ...]
    robot_body_link_names: tuple[str, ...]
    robot_body_indices_device: Any
    robot_body_com: np.ndarray
    robot_urdf_model: URDFModel
    robot_world_transform: np.ndarray
...
class NewtonPhysicsAssistedSimulator:
    ...
    def _build_runtime(
        ...
        try:
            robot_urdf_model = load_urdf(params.robot_urdf)
            robot_world_transform = pose_matrix(
            ...
            with wp.ScopedDevice(params.device):
                ...
                if params.fixed_grasp_enabled:
                    ...
                    grasp_joint_index = builder.add_joint_fixed(
                        parent=ee_body,
                        child=handle_body,
                ...
                object_body_indices_ordered = tuple(sorted(object_body_indices))
                robot_body_link_names = _resolve_robot_body_link_names(
                    model.body_label,
                    robot_body_indices_ordered,
                    robot_urdf_model,
                )

                # Convert only imported robot bodies to prescribed kinematic
                # bodies.  Object flags and inertial arrays are snapshotted and
                # verified untouched by this indexed mutation.
                body_flags = model.body_flags.numpy().astype(np.int32, copy=True)
                inv_mass = model.body_inv_mass.numpy().astype(float, copy=True)
                inv_inertia = model.body_inv_inertia.numpy().astype(float, copy=True)
                object_flags_before = body_flags[list(object_body_indices_ordered)].copy()
                object_inv_mass_before = inv_mass[list(object_body_indices_ordered)].copy()
                object_inv_inertia_before = inv_inertia[
                    list(object_body_indices_ordered)
                ].copy()
                body_flags[list(robot_body_indices_ordered)] = int(
                    newton.BodyFlags.KINEMATIC
                )
                inv_mass[list(robot_body_indices_ordered)] = 0.0
                inv_inertia[list(robot_body_indices_ordered)] = 0.0
                model.body_flags.assign(
                    wp.array(body_flags, dtype=wp.int32, device=model.device)
                )
                model.body_inv_mass.assign(
                    wp.array(inv_mass, dtype=wp.float32, device=model.device)
                )
                model.body_inv_inertia.assign(
                    wp.array(inv_inertia, dtype=wp.mat33, device=model.device)
                )
                verified_flags = model.body_flags.numpy()
                verified_inv_mass = model.body_inv_mass.numpy()
                verified_inv_inertia = model.body_inv_inertia.numpy()
                if not (
                    np.all(
                        verified_flags[list(robot_body_indices_ordered)]
                        == int(newton.BodyFlags.KINEMATIC)
                    )
                    and np.all(
                        verified_inv_mass[list(robot_body_indices_ordered)] == 0.0
                    )
                    and np.all(
                        verified_inv_inertia[list(robot_body_indices_ordered)] == 0.0
                    )
                ):
                    raise PipelineError(
                        FailureCode.PHYSICS_UNAVAILABLE,
                        "robot bodies were not finalized as zero-inertia kinematic bodies",
                        stage="physics_model",
                    )
                if not (
                    np.array_equal(
                        verified_flags[list(object_body_indices_ordered)],
                        object_flags_before,
                    )
                    and np.array_equal(
                        verified_inv_mass[list(object_body_indices_ordered)],
                        object_inv_mass_before,
                    )
                    and np.array_equal(
                        verified_inv_inertia[list(object_body_indices_ordered)],
                        object_inv_inertia_before,
                    )
                ):
                    raise PipelineError(
                        FailureCode.NUMERICAL_INSTABILITY,
                        "kinematic robot setup modified object body properties",
                        stage="physics_audit",
                    )
                ...
                robot_body_indices_device = wp.array(
                    np.asarray(robot_body_indices_ordered, dtype=np.int32),
                    dtype=wp.int32,
                    device=model.device,
                )
                robot_body_com = model.body_com.numpy()[
                    list(robot_body_indices_ordered)
                ...
                return _PhysicsRuntime(
                    ...
                    robot_body_indices_ordered=robot_body_indices_ordered,
                    robot_body_link_names=robot_body_link_names,
                    robot_body_indices_device=robot_body_indices_device,
                    robot_body_com=robot_body_com,
                    robot_urdf_model=robot_urdf_model,
                    robot_world_transform=robot_world_transform,
            ...
            raise PipelineError(
                FailureCode.ASSET_INVALID,
                "robot URDF could not be loaded for the kinematic body driver",
                stage="physics_model",
                details=exc.to_dict(),
            ) from exc
    ...
    def run(self, commands: PhysicsCommandTrajectory) -> PhysicsRollout:
        ...
        previous_robot_poses = named_robot_body_poses(
            runtime.robot_urdf_model,
            runtime.robot_world_transform,
            runtime.robot_body_link_names,
            params.arm_joint_names,
            commands.arm_joint_targets[0],
            params.finger_joint_names,
            float(commands.gripper_width_m[0]),
        ...
        try:
            for frame_index in range(frame_count):
                ...
                for substep_index in range(params.substeps):
                    ...
                    current_robot_poses = named_robot_body_poses(
                        runtime.robot_urdf_model,
                        runtime.robot_world_transform,
                        runtime.robot_body_link_names,
                        params.arm_joint_names,
                    ...
                    robot_twists = kinematic_body_twists(
                        previous_robot_poses,
                        current_robot_poses,
                        runtime.robot_body_com,
                        substep_dt,
                    )
                    if _scatter_indexed_body_state is None:
                        raise PipelineError(
                            FailureCode.PHYSICS_UNAVAILABLE,
                            "indexed kinematic body scatter kernel is unavailable",
                            stage="physics_control",
                        )
                    pose_rows = np.stack(
                        [
                            _newton_pose_row_from_matrix(transform)
                            for transform in current_robot_poses
                        ]
                    ).astype(np.float32)
                    wp.launch(
                        kernel=_scatter_indexed_body_state,
                        dim=len(runtime.robot_body_indices_ordered),
                        inputs=[
                            runtime.robot_body_indices_device,
                            wp.array(
                                pose_rows,
                                dtype=wp.transform,
                            ...
                            wp.array(
                                robot_twists.astype(np.float32),
                                dtype=wp.spatial_vector,
                                device=params.device,
                            ),
                        ],
                        outputs=[
                            runtime.state_in.body_q,
                            runtime.state_in.body_qd,
                        ],
                        device=params.device,
                    )
                    ...
                    previous_robot_poses = current_robot_poses

                # XPBD evolves only dynamic body state.  Generalized
                ...
                # These are the coordinates that generated the prescribed FK
                # poses.  eval_ik is authoritative for the dynamic door, but
                # is underdetermined for a chain whose every body is kinematic.
                arm_q[frame_index] = commands.arm_joint_targets[frame_index]
        ...
        metadata: dict[str, Any] = {
            ...
            "robot_control_semantics": "prescribed_name_based_urdf_fk_body_poses_not_torque_pd",
            "robot_joint_coordinate_semantics": "kinematic_driver_input_q_not_eval_ik",
            "door_coordinate_measurement_semantics": "newton_eval_ik_from_dynamic_body_state",
            "robot_body_state_write_backend": "indexed_scatter_robot_body_q_and_body_qd_only",
            "robot_target_write_backend": "none",
            ...
            "robot_body_indices_written": list(runtime.robot_body_indices_ordered),
            "robot_body_link_names_written": list(runtime.robot_body_link_names),
            "object_body_indices_untouched_by_driver": sorted(
            ...
            "robot_body_inverse_mass_zero_verified": True,
            "robot_body_inverse_inertia_zero_verified": True,
            "robot_body_flags_kinematic_verified": True,
            "constraint_backend": (
...
def _validate_physics_rollout(
    ...
    if arrays["robot_driver_arm_joint_q"].shape != expected_arm_shape:
        raise PipelineError(
            FailureCode.PHYSICS_UNAVAILABLE,
            "kinematic driver arm positions do not match configured arm commands",
            stage="physics_result",
            details={"expected": list(expected_arm_shape), "actual": list(arrays["robot_driver_arm_joint_q"].shape)},
        )
    ...
    if np.any(arrays["external_robot_joint_force_command"] != 0.0):
        raise PipelineError(
            FailureCode.NUMERICAL_INSTABILITY,
            "kinematic body driver reported a non-zero robot joint-force command",
            stage="physics_audit",
        )
    ...
    if metadata.get("door_actuation") != "none":
        raise PipelineError(
            FailureCode.NUMERICAL_INSTABILITY,
            "physics backend did not prove that door actuation is disabled",
            stage="physics_audit",
    ...
    if metadata.get("control_backend") != "kinematic_body_driver":
        raise PipelineError(
            FailureCode.NUMERICAL_INSTABILITY,
            "physics backend is not the configured kinematic body driver",
            stage="physics_audit",
            details={"control_backend": metadata.get("control_backend")},
        )
    for field in (
        "door_coord_excluded_from_driver",
        "door_dof_excluded_from_driver",
        "robot_object_body_index_sets_disjoint",
    ):
        if metadata.get(field) is not True:
            raise PipelineError(
                FailureCode.NUMERICAL_INSTABILITY,
                "indexed kinematic driver exclusion assertion is missing",
                stage="physics_audit",
        ...
        rows.append(
            {
                ...
                "arm_joint_positions": {
                    ...
                    for name, value in zip(
                        arm_joint_names,
                        physics["robot_driver_arm_joint_q"][index],
                        strict=True,
                    )
                },
...
def run_physics_assisted(
    ...
    try:
        computed = compute_metrics(
            ...
            joint_q=physics["robot_driver_arm_joint_q"],
    ...
    trajectory = {
        ...
        "arm_joint_q": physics["robot_driver_arm_joint_q"],
        "joint_lower": joint_lower,
        "joint_upper": joint_upper,
        "ik_success_flags": reference_arrays["ik_success_flags"],
