...
def _output_directory(config: ProjectConfig, override: str | Path | None) -> Path:
    if override is None:
        return config.resolve_path(str(config.get("output.root")))
...
def _default_collision_factory(
    ...
    try:
        from .collision import build_collision_evaluator  # type: ignore[attr-defined]
    ...
    return build_collision_evaluator(config, kinematics, backend)
...
class _DisabledCollisionEvaluator:
    ...
    def candidate_is_collision_free(
        self, candidate: GraspCandidate, gripper_world: np.ndarray
    ) -> CheckResult:
        del candidate, gripper_world
...
def _joint_violation_row(item: Any) -> dict[str, Any]:
    return {
        ...
        "lower": float(getattr(item, "lower")),
        "upper": float(getattr(item, "upper")),
        "magnitude": float(getattr(item, "magnitude")),
    }
        ...
        rows.append(
            {
                ...
                "handle_world": plan.handle_world[frame_index],
                "target_gripper_world": plan.target_gripper_world[frame_index],
                "achieved_gripper_world": achieved_gripper_world[frame_index],
                "arm_joint_positions": {
                    name: float(value)
                ...
                "ik": {
                    ...
                    "objective_cost": float(getattr(waypoint, "objective_cost")),
                    "position_error_m": float(getattr(pose_error, "position_m")),
...
def run_kinematic(
    ...
    *,
    ...
    log = _RunLog(mode="kinematic", seed=int(config.get("seed")))
    ...
    selection = select_grasp_candidate(
        ...
        reachability_check=lambda candidate: _single_target_is_reachable(
            config=config,
            backend=backend,
            gripper_world=target_for(candidate),
        ),
        collision_free_check=lambda candidate: collision_evaluator.candidate_is_collision_free(
            candidate, target_for(candidate)
        ),
    )
    write_json(
        destination / "affordance_candidates.json",
        {"resolution": resolution.to_dict(), "selection": selection.to_dict()},
    )
    log.add(
    ...
    collision_flags = _validate_collision_flags(
        collision_evaluator.trajectory_collision_flags(plan, arm_joint_q),
        len(plan.phase_names),
    )
    ...
    if _bool_config(config, "output.write_resolved_config"):
        write_json(destination / "resolved_config.json", config.data)
    log.add(
        "run_completed",
        status=run_status,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        destination = _output_directory(config, destination)
        ...
        if destination is not None:
            try:
                ...
                pass
        print(json.dumps({"success": False, "failure": failure}, ensure_ascii=False, sort_keys=True), file=sys.stderr)
        return 2
